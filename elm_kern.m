function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,Y,TY,OutputWeight] = elm_kern(Omega_train, train_labels, Omega_test, test_labels, Elm_Type, Regularization_coefficient,wtELM)
...
if (nargin<7 || Elm_Type==REGRESSION)
    wtELM=0;
end
...
if Elm_Type~=REGRESSION
    ...
    temp_T=zeros(NumberofOutputNeurons, Ntr);
    for i = 1:Ntr
    ...
    if (wtELM)
        W=speye(Ntr);
        ClassHistogram=histc(train_labels,1:number_class);
      
        iHistogram=1./ClassHistogram;
        for i = 1:length(train_labels)
             W(i,i)=iHistogram(train_labels(i));
        end
    end
    %%%%%%%%%% Processing the targets of testing
    temp_TV_T=zeros(NumberofOutputNeurons, NTs);
    for i = 1:NTs
        for j = 1:number_class
...
%%%%%%%%%% Calculate training & testing classification accuracy
     ...
     [~, label_index_predicted]=max(Y);
     [~, label_index_actual]=max(T);
     MissClassificationRate_Training=sum(label_index_actual~=label_index_predicted);
%     for i = 1 : size(T, 2)
...
%     end
     ...
     [~, label_index_predicted]=max(TY);
     [~, label_index_actual]=max(TV.T);
     MissClassificationRate_Testing=sum(label_index_actual~=label_index_predicted);
%     for i = 1 : size(TV.T, 2)
...
%     end
    ...
    [UAR,Accuracy]=getUAR(label_index_actual,label_index_predicted);
    
    if (wtELM)
        fprintf('WELM Accuracy= %f, UAR=%f\n',Accuracy,UAR);
    else
        fprintf('ELM Accuracy= %f, UAR=%f\n',Accuracy,UAR);
    end
end
